...
regression tests. `R CMD check` is clean. Four vignettes cover the
user-facing geoms, choropleth mapping, the built-in pattern reference, and
design philosophy.
...
between pattern elements on every shape at every size. Grid resolves mm to
device pixels inside the active shape viewport at draw time via
`makeContent`-based deferral, giving uniform visual density across all shapes
in the same plot regardless of their physical size.
...
visual density in the data area.
...
- ~~`pattern_spacing` changes from a dimensionless bbox fraction to millimetres.
  ...
  default `0.5` (mm).~~ — shipped in v0.6.0.
- ~~`makeContent`-based deferral (`DotPatternTree`, `LinePatternTree` grob
  classes) so mm → npc conversion happens inside the active shape viewport at
  draw time.~~ — shipped in v0.6.0.
- ~~Named density variants (`hatch_dense` etc.) bake in a spacing multiplier
  against the mm default — no changes needed to variant registrations.~~ —
  shipped in v0.6.0.
