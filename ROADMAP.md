...
`geom_polygon_pattern` clips patterns to the exact polygon shape using
device-independent in-R computation (`clip_segments_to_poly` + `pip`).
This works on all devices and R versions, but it only handles simple
...
`R/aaa_draw_key.R` multiplies `pattern_spacing` by 2.5 in legend keys so
the swatch isn’t a dense blob. This means the legend swatch doesn’t
faithfully represent the pattern density used in the data area. A better
approach would compute a target swatch-relative spacing instead of a
