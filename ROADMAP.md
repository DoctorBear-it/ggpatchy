...
visual regression tests. `R CMD check` is clean. Three vignettes cover
the user-facing geoms, choropleth mapping, and the built-in pattern
reference.
...
8% of the shape’s bounding box. This is consistent within a single shape
but does **not** guarantee uniform visual density across shapes of
different physical sizes in the same plot — see the top roadmap item.
...
**This is the most visible failure mode in production plots.**
`pattern_spacing` is currently a fraction of each shape’s bounding box.
A large state (Texas) and a small state (Rhode Island) with the same
`pattern_spacing` value get very different dot counts and line
frequencies — not because the user asked for this, but because their
bounding boxes differ by an order of magnitude. The dots pattern is
especially punishing because absolute count differences read
immediately.

The fix is physical-unit spacing: `pattern_spacing` expressed in
millimetres so that `spacing = 5` means “5mm between elements” on every
shape at every size. Grid resolves mm to device pixels at draw time,
independently of viewport size. This is the correct model and the only
one that gives the user meaningful, predictable control.

The spec for this work exists at `specs/SPEC_physical_unit_spacing.md`.
It is a breaking change to `pattern_spacing` semantics (dimensionless →
mm) and requires a `makeContent`-based deferral to convert mm → npc
inside the active viewport at draw time. Target: 0.6.0.

**The user contract:** at default `pattern_spacing`, all standard
patterns should look the same on a reference output (A4 at 150 DPI,
typical chart element). Any deviation from that baseline is explicitly
the user’s choice via `pattern_spacing`. The pattern-reference vignette
(`vignettes/pattern-reference.Rmd`) establishes this baseline and serves
as the visual definition of “correct”.
...
faithfully represent the visual density in the data area. When the
physical-unit spacing fix lands (0.6.0), the legend will automatically
be correct because mm spacing resolves identically in both contexts.
...
- `pattern_spacing` changes from a dimensionless bbox fraction to
  ...
  dimensionless to mm, default `0.5` (mm).
- Implementation: `makeContent`-based deferral (`DotPatternTree`,
  `LinePatternTree` custom grob classes) so mm → npc conversion happens
  inside the active shape viewport at draw time, not in the pattern
  function body.
- All `default_aes` in all geom files updated. All
  `@param pattern_spacing` docs updated. All snapshots regenerated.
- `vignettes/design-philosophy.Rmd` rewritten to describe the mm model.
- Named density variants (`hatch_dense` etc.) automatically work
  correctly because they bake in a spacing multiplier against whatever
  the default is — no changes needed to variant registrations.
- See `specs/SPEC_physical_unit_spacing.md` for the full implementation
  spec.
